...
import com.functorz.floatwindow.FloatWindowManager
import com.functorz.floatwindow.DefaultFloatService
import com.functorz.floatwindow.listener.OnClickListener
import com.functorz.worktool.Constant
import com.functorz.worktool.R
import com.functorz.worktool.activity.ListenActivity
import com.functorz.worktool.model.WeworkMessageBean
import com.functorz.worktool.service.*
import java.io.File
...
object FloatWindowHelper {
    ...
    private fun accessibilityServiceResume() {
        if (PermissionHelper.isAccessibilitySettingOn()) {
            ...
            //隐藏软键盘模式
            WeworkController.weworkService.softKeyboardController.showMode =
            ...
            MyLooper.getInstance().sendMessage(Message.obtain().apply {
                ...
                obj = WeworkMessageBean()
                    .apply { type = WeworkMessageBean.LOOP_RECEIVE_NEW_MESSAGE }
            })
        } else {
            ...
            service.onClickListener = object : OnClickListener {
                override fun onClick(v: View, event: Int) {
                    when (event) {
                                ...
                                thread {
                                    ...
                                    if (create && newFile.canWrite()) {
                                        ...
                                        LogUtils.i(
                                            ...
                                            "当前页面: ${WeworkController.weworkService.currentClass}"
                                        )
                                    }
                        ...
                        3 -> {
                            Utils.getApp().packageManager.getLaunchIntentForPackage(
                                Constant.PACKAGE_NAMES
                            )?.apply {
